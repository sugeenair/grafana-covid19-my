...
covid_moh_git="https://raw.githubusercontent.com/MoH-Malaysia/covid19-public/main/epidemic/clusters.csv"
my_covid_file="clusters.csv"
wget=$(which wget)
db_name="covid19_MY"
table_name="my_covid_clusters"
db_user="gadmin"
datafile=/var/lib/mysql-files/clusters.csv
countfile="cluster_cnt.txt"
...
use covid19_MY
select count(*) from my_covid_clusters
EOF
...
function extract_transform() {
	cat $my_covid_file | sed 's/WP //g' > temp.csv
...
function load_db() {
        mysql --defaults-extra-file=$extra -e "use $db_name" -e "
          LOAD DATA INFILE '$datafile'
           ...
           set 
	   summary_bm = NULLIF(@summary_bm,''),
	   summary_en = NULLIF(@summary_en,''),
	   c_time = now();"
